...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        if (auth()->user()->isBendahara() || auth()->user()->isStafKeuangan()) {
            return redirect()->route('keuangan.dashboard');
        }

        $academicService = new \App\Services\AcademicService();
        $studentService = new \App\Services\StudentService($academicService);

        $activeYear = $academicService->activeYear();
        if (!$activeYear) {
        ...
        $stats = [
            'total_siswa' => $studentStats['total'],
            'siswa_mi' => $studentStats['mi'],
            'siswa_mts' => $studentStats['mts'],
