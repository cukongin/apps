...
class MaintenanceController extends Controller
{
    ...
    public function magicFix(Request $request)
    {
         ...
         try {
             ...
             // 3. Fix Class Level (Jenjang) - ROBUST VERSION
             $this->fixClassLevel($request); // Reuse the robust logic
             $log[] = "Identitas Kelas & Jenjang diperbaiki (Smart fix).";
    ...
    public function fixClassLevel(Request $request)
    {
        ...
        foreach ($classes as $kelas) {
            ...
            // Robust Detection Logic (Regex)
            // Matches: TPQ, T.P.Q
            if (preg_match('/t\.?p\.?q\.?/i', $name)) {
                $newJenjangId = DB::table('jenjang')->where('kode', 'TPQ')->value('id');
                if ($newJenjangId && $kelas->id_jenjang != $newJenjangId) {
                    $kelas->id_jenjang = $newJenjangId;
                    $kelas->save();
                    $countTPQ++;
                }
            }
            // Matches: MTS, M.Ts, M.T.S, M Ts, MTs. (Allow spaces)
            elseif (preg_match('/m\s*\.?\s*t\s*\.?\s*s\.?/i', $name)) {
                $newJenjangId = $mtsId;
            ...
            elseif (stripos($name, 'MI') !== false) {
                ...
                // Fallback: Number based ONLY if no specific keyword found
                if (in_array($firstChar, ['1', '2', '3', '4', '5', '6'])) {
                    $newJenjangId = $miId; // Assume MI default for 1-6
                    if ($kelas->id_jenjang != $newJenjangId) {
                        $kelas->id_jenjang = $newJenjangId;
                        $kelas->save();
                        $countMI++;
            ...
            // Fix Tingkat (Grade Level)
            if (is_numeric($firstChar)) {
                $tingkat = (int)$firstChar;

                // SUPER LOGIC: Map 1/2/3 MTs -> 7/8/9
                if ($kelas->id_jenjang == $mtsId) {
                    if ($tingkat == 1) $tingkat = 7;
                    elseif ($tingkat == 2) $tingkat = 8;
                    elseif ($tingkat == 3) $tingkat = 9;
                }

                if ($kelas->tingkat_kelas != $tingkat) {
                    $kelas->tingkat_kelas = $tingkat;
                    $kelas->save();
                }
            }
        }

        return back()->with('success', "Terapi Identitas Selesai. $countMI kelas MI, $countMTS kelas MTS, dan $countTPQ kelas TPQ berhasil diperbaiki.");
    }
    ...
    public function resetSystem(Request $request)
    {
        ...
        try {
            ...
            // 1b. Finance Transactional Data (Unified)
