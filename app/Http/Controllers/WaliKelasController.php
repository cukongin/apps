...
class WaliKelasController extends Controller
{
    private function getWaliKelasInfo()
    {
        ...
        $service = new \App\Services\AcademicService();
        $activeYear = $service->activeYear();

        if (($user->isAdmin() || $user->isTu()) && request('year_id')) {
             $requestedYear = TahunAjaran::find(request('year_id'));
             if($requestedYear) $activeYear = $requestedYear;
        }

        if (!$activeYear) abort(404, 'Tahun Ajaran Aktif tidak ditemukan.');
        ...
        if ($kelas) {
            ...
            if (!$periode) {
                 // Use Service if checks match active year (otherwise manual for history)
                 if ($activeYear->status === 'aktif') {
                     $periode = $service->activePeriod($kelas->jenjang->kode);
                 } else {
                     // History Mode: Try to find "Genap" or last active-like period for that year?
                     // Or just first one found?
                     $periode = Periode::where('id_tahun_ajaran', $activeYear->id)
                        ->where('lingkup_jenjang', $kelas->jenjang->kode)
                        ->orderBy('id', 'desc') // Last one (usually Genap/Cawu 3)
                        ->first();
                 }
            }
    ...
    public function dashboard(Request $request)
    {
        ...
        if ($periode) {
            ...
            $gradingSettings = \App\Models\Jenjang::getSettings($kelas->jenjang->kode);
            $defaultKkm = $gradingSettings->kkm_default;
    ...
    public function storeAbsensi(Request $request)
    {
        ...
        // if ($kelas->id_wali_kelas !== Auth::id() && !Auth::user()->isAdmin() && !Auth::user()->isTu()) {
        ...
        try {
            ...
            DB::commit();
            return back()->with('success', 'Data absensi & kepribadian berhasil disimpan secara permanen.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    $allClasses = collect([]);
    if (Auth::user()->isAdmin() || Auth::user()->isTu()) {
        $activeYear = $this->getWaliKelasInfo()[2]; // Get activeYear from helper
        $query = Kelas::where('id_tahun_ajaran', $activeYear->id)
            ->whereHas('jenjang', function($q) {
                $q->where('has_rapor', true);
            });
    ...
    return view('wali-kelas.ekskul', compact('kelas', 'periode', 'students', 'ekskulRows', 'ekskulOptions', 'allClasses'));
    }
    ...
    public function kenaikanKelas()
    {
        ...
        if (Auth::user()->isAdmin() || Auth::user()->isTu()) {
            // Re-fetch Active Year/Period just in case getWaliKelasInfo failed (e.g. admin has no class)
            if (!$activeYear) $activeYear = \App\Models\TahunAjaran::where('status', 'aktif')->first();
            if (!$systemActivePeriod) $systemActivePeriod = Periode::where('id_tahun_ajaran', $activeYear->id)->where('status', 'aktif')->first();
        ...
        if (!$kelas) {
            return back()->with('error', 'Kelas tidak ditemukan atau Anda tidak memiliki akses.');
        }
        ...
        if ($activeP) {
            ...
            if (str_contains($name, 'cawu 3') || str_contains($name, 'semester 2') || str_contains($name, 'genap') || str_contains($name, 'akhir') || str_contains($name, 'pat') || str_contains($name, 'ukk')) {
                $isFinalPeriod = true;
            }
        ...
        $maxKkmFailure = (int) $gradingSettings->promotion_max_kkm_failure;

        $minAttendance = (float) $gradingSettings->promotion_min_attendance;
        ...
        // 3. Get All Grades for Active Year (By Students, NOT Class ID - to handle transfers/changes)
        $studentIds = $students->pluck('student.id'); // Verify if 'student' relation loaded, or use 'id_siswa'
        // $students is collection of AnggotaKelas -> has 'id_siswa'
        ...
        // 5. Get Attendance (By Students)
        $allAttendance = CatatanKehadiran::whereIn('id_siswa', $studentIds)
            ->whereIn('id_periode', $periods->pluck('id'))
            ->get();
        ...
        // OPTIMIZATION: Get Admin IDs for Lock Check
        $adminIds = \App\Models\User::whereIn('role', ['admin', 'staff_tu', 'kepala_madrasah'])->pluck('id')->toArray();
        ...
        foreach ($students as $student) {
            ...
            foreach ($assignedMapels as $mapel) {
                ...
                if ($isFinalPeriod) {
                    // FINAL: Cumulative Failures Logic (Requested by User)
                    // Count how many times this mapel failed across ALL periods
                    // "Cawu 1=3, Cawu 2=2, Cawu 3=0 -> Total 5"

                    // 1. Calculate Yearly Avg for "Nilai Akhir" display
                    $theseGrades = $sGrades->where('id_mapel', $mId);
                    $finalMapelScore = $theseGrades->avg('nilai_akhir') ?? 0;

                    // 2. Count Failures across periods
                    foreach ($periods as $p) {
                         $pGrade = $theseGrades->where('id_periode', $p->id)->first();
                         if ($pGrade && $pGrade->nilai_akhir < $kkm) {
                             $underKkmCount++;
                         }
                    }
                } elseif (isset($activeP)) {
                ...
                if ($isFinalPeriod) {
                    foreach ($periods as $p) {
                         $pGrade = $theseGrades->where('id_periode', $p->id)->first();
                         $val = $pGrade ? $pGrade->nilai_akhir : '-';
                         $isFail = ($pGrade && $pGrade->nilai_akhir < $kkm);
                         if ($isFail) $accFailures++;
                ...
                $gradesDetail[] = [
                    ...
                    'has_failure' => ($accFailures > 0 || ($finalMapelScore < $kkm)) // Flag for ANY failure
                ];
            }
            ...
            // 1. CEK STRICT: JUMLAH MAPEL (Anti-Pass if No Data)
            $mapelCount = $sGrades->pluck('id_mapel')->unique()->count();
            if ($mapelCount == 0) {
            ...
            if ($isFinalPeriod) {
                // FINAL PERIOD: Use Configurable Limit (Syarat Kenaikan Kelas)

                // IMPLEMENTATION: SEPARATE GRADUATION VS PROMOTION
                if ($isFinalYear) {
                     // GRADUATION (Kelas 6/9):
                     // Ignore cumulative mapel failures for the "Decision" (allow graduation logic to proceed).
                     // The "Ijazah Note" section below will still warn about remedials if passed.
                } else {
                     // PROMOTION (Kelas 1-5 / 7-8):
                     // STRICT: Cumulative Failures > Limit = Stay
                     if ($underKkmCount > $maxKkmFailure) $failConditions[] = "Mapel < KKM ($underKkmCount > $maxKkmFailure)";
                }
            } else {
                // NON-FINAL PERIOD: STRICT (Any Remedial = Belum Tuntas)
                if ($underKkmCount > 0) $failConditions[] = "Ada $underKkmCount Mapel Remedial";
            }
            ...
            // Override Logic (AUTO-SYNC FIX)
            $decisionObj = $decisions[$sId] ?? null;
            $isLockedByAdmin = $decisionObj && $decisionObj->override_by && in_array($decisionObj->override_by, $adminIds);
            $isDecisionLocked = $decisionObj && !is_null($decisionObj->override_by);

            // CRITICAL FIX: If NOT locked, use System Recommendation (Auto-Sync)
            // This prevents "Stale" data from showing "Naik Kelas" when rules say "Tinggal Kelas"
            if ($isDecisionLocked) {
                $finalStatus = $decisionObj->final_decision;
            } else {
            ...
            if ($isFinalYear) {
                 ...
                 if ($iCount > 0) {
                     ...
                     // FIX: Gunakan Setting Per-Jenjang (MI/MTS)
                     ...
                     if ($iAvg >= $minLulusIjazah) {
                         ...
                         $ijazahClass = "text-red-600 font-bold";
                         $failConditions[] = "Nilai Ijazah Kurang";
                     }
                 } else {
                      // Belum ada nilai Ijazah
                      $ijazahNote = "⚠️ Nilai Ijazah Belum Lengkap / Belum Ada";
                      $ijazahClass = "text-slate-400 font-bold italic";
                      // Do not force fail if just missing, unless required?
                      // Usually "Belum Ada Nilai" is handled by Mapel Count check, but for Ijazah specifically:
                      if($isFinalPeriod) $failConditions[] = "Nilai Ijazah Kosong";
                 }
            } else {
                 // --- Regular Class Logic ---
                 // User Request: "tidak sampai KKM juga harus ada info"

                 // Get Failed Mapels (if any) - FIX: Use 'has_failure' to capture cumulative failures
                 $failedMapels = collect($gradesDetail)->where('has_failure', true)->pluck('mapel');

                 if ($underKkmCount > 0) {
                     $count = $underKkmCount; // Use the accumulated count directly
            ...
            $studentStats[] = (object) [
                ...
                'is_locked_by_admin' => $isLockedByAdmin, // NEW: For Read-Only UI
                'manual_note' => ($manualNote && !str_starts_with($manualNote, 'Perhatian') && !str_starts_with($manualNote, 'Sikap') && !str_starts_with($manualNote, 'Kehadiran') && !str_starts_with($manualNote, 'Mapel') && (
                    // Only filter "Memenuhi" if status is Retained/Not Graduated
                    !str_starts_with($manualNote, 'Memenuhi semua syarat') || (in_array($finalStatus, ['promoted', 'promote', 'graduated', 'graduate']))
                )) ? $manualNote : null, // Filter out legacy system notes
    ...
    public function storeKenaikanKelas(Request $request)
    {
        ...
        // Context
        // If Admin, they might be editing a different class
        if (Auth::user()->isAdmin() || Auth::user()->isTu()) {
            $classId = $request->input('class_id'); // Must be passed hidden
            $kelas = Kelas::find($classId);
            $activeYear = \App\Models\TahunAjaran::where('status', 'aktif')->firstOrFail();
             // Period?
             $periode = Periode::where('id_tahun_ajaran', $activeYear->id)->where('status', 'aktif')->first();
        } else {
             list($kelas, $periode, $activeYear) = $this->getWaliKelasInfo();
        }
        ...
        // OPTIMIZATION: Get Admin IDs
        $adminIds = \App\Models\User::whereIn('role', ['admin', 'staff_tu', 'kepala_madrasah'])->pluck('id')->toArray();
        $isUserAdmin = in_array(Auth::id(), $adminIds);
        ...
        foreach ($decisions as $sId => $decision) {
            ...
            $isLockedByAdmin = $existing && $existing->override_by && in_array($existing->override_by, $adminIds);

            // If Locked by Admin AND Current User is NOT Admin -> Block
            if ($isLockedByAdmin && !$isUserAdmin) {
                continue; // Skip silently or warn? Skip for now.
            }
            ...
            // IF ADMIN: Force Lock (Set Override)
            // IF WALIKELAS: Set Override (Own)
            $data['override_by'] = Auth::id();

            DB::table('promotion_decisions')->updateOrInsert(
                [
                    'id_siswa' => $sId,
                    'id_kelas' => $kelas->id, // Use ID from helper/input
                    'id_tahun_ajaran' => $activeYear->id
                ],
                $data
            );

            // No changes to CatatanWaliKelas logic
            // ...
        }
    ...
    {
        if (!Auth::user()->isAdmin() && !Auth::user()->isTu()) return back()->with('error', 'Akses Ditolak');

        $classId = $request->class_id;
        $activeYear = \App\Models\TahunAjaran::where('status', 'aktif')->firstOrFail();

        // 1. Force Calculate (Ensure records exist)
        // Resolve PromotionController to reuse logic
        $promoCtrl = app(\App\Http\Controllers\PromotionController::class);
        $promoCtrl->calculate($classId);

        // 2. Lock (Set override_by = Auth::id())
        ...
        // 3. Sync to Buku Induk (Optional but good)
        // $promoCtrl->finalize($request); // finalize might lock ALL classes or just one?
        // PromotionController::finalize operates on ALL records for the year. Too dangerous.
        // We just stick to locking the decision.

        return back()->with('success', "Kelas berhasil DIKUNCI. $affected siswa diamankan dari perubahan Wali Kelas.");
    }

    public function unlockClass(Request $request)
    {
        if (!Auth::user()->isAdmin() && !Auth::user()->isTu()) return back()->with('error', 'Akses Ditolak');
        ...
        // Unlock: Set override_by = NULL
        // CAUTION: Resets decisions to System Recommendation
        $affected = DB::table('promotion_decisions')
            ->where('id_kelas', $classId)
            ->where('id_tahun_ajaran', $activeYear->id)
            ->update([
        ...
        return back()->with('success', "Kelas berhasil DIBUKA KEMBALI. $affected siswa dikembalikan ke Rekomendasi Sistem.");
    }
