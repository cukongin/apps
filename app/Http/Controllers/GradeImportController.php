...
class GradeImportController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $gradingSettings = \App\Models\Jenjang::getSettings($activeJenjang);
    ...
    public function previewGlobal(Request $request, $jenjang)
    {
        ...
        foreach ($headerRow as $idx => $col) {
            if (preg_match('/\[(\d+)\]/', $col, $matches)) {
                ...
                // Fallback if not found in string
                if ($pId === 0 || $pId === $allPeriods->first()->id) {
                     // Check if we can be smarter, or just stick to default
                     // If header has NO period suffix, and we have multiple periods, this is risky.
                     // But usually template generation matches this logic.
                     if ($pId === 0 && $allPeriods->count() > 0) {
                        $activeP = $allPeriods->firstWhere('status', 'aktif');
                        $pId = $activeP ? $activeP->id : $allPeriods->first()->id;
                     }
                }
    ...
    public function storeGlobal(Request $request)
    {
        ...
        $grading = \App\Models\Jenjang::getSettings($jenjang);
