                                ...
                                <div class="flex items-center gap-2 text-xs text-slate-500">
                                    ...
                                    <div class="flex gap-1">
                                        @foreach($menu->roles as $role)
                                            @php
                                                $badgeClass = match($role->role) {
                                                    'admin' => 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400',
                                                    'teacher' => 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400',
                                                    'bendahara' => 'bg-pink-100 text-pink-600 dark:bg-pink-900/30 dark:text-pink-400',
                                                    'staf_keuangan' => 'bg-teal-100 text-teal-600 dark:bg-teal-900/30 dark:text-teal-400',
                                                    'staff_tu' => 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400',
                                            ...
                                            @endphp
                                            <span class="px-1.5 py-0.5 rounded {{ $badgeClass }}">{{ $role->role }}</span>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
            ...
            <form :action="isEdit ? '{{ url('settings/menus') }}/' + form.id : '{{ route('settings.menus.store') }}'" method="POST" class="p-6 space-y-6">
                ...
                <div class="space-y-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">
                    ...
                    <div class="flex flex-wrap gap-6">
                        ...
                        <label class="flex items-center gap-2 cursor-pointer hover:text-primary transition-colors">
                            <input type="checkbox" name="roles[]" value="bendahara" x-model="form.roles" class="rounded border-slate-300 text-pink-600 focus:ring-pink-600 w-5 h-5">
                            <span class="text-sm font-semibold text-pink-700">Bendahara</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer hover:text-primary transition-colors">
                            <input type="checkbox" name="roles[]" value="staf_keuangan" x-model="form.roles" class="rounded border-slate-300 text-teal-600 focus:ring-teal-600 w-5 h-5">
                            <span class="text-sm font-semibold text-teal-700">Staf Keuangan</span>
                        </label>
                    </div>
                </div>
