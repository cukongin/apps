        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="card-boss p-6 relative overflow-hidden group">
                    <div class="absolute -right-6 -top-6 opacity-5 group-hover:opacity-10 transition-opacity transform group-hover:rotate-12 duration-500">
                        <span class="material-symbols-outlined text-9xl text-primary">groups</span>
                    </div>
                    <div class="flex flex-col gap-1 relative z-10">
                        <div class="flex items-center gap-2 mb-2">
                             ...
                             <p class="text-xs font-bold uppercase tracking-wider text-slate-500">Total Santri</p>
                        </div>
                        <h3 class="text-4xl font-extrabold text-slate-800 dark:text-white">{{ $stats['total_siswa'] }}</h3>
                        <div class="flex gap-2 mt-2">
                            <span class="px-2 py-1 rounded-md bg-emerald-50 text-emerald-700 text-[10px] font-bold border border-emerald-100 dark:bg-emerald-900/20 dark:border-emerald-800 dark:text-emerald-400">
                                MI: {{ $stats['siswa_mi'] }}
                            </span>
                            <span class="px-2 py-1 rounded-md bg-blue-50 text-blue-700 text-[10px] font-bold border border-blue-100 dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-400">
                                MTs: {{ $stats['siswa_mts'] }}
                            </span>
                        </div>
                    </div>
                ...
                <div class="card-boss p-6 relative overflow-hidden group">
                     <div class="absolute -right-6 -top-6 opacity-5 group-hover:opacity-10 transition-opacity transform group-hover:rotate-12 duration-500">
                        <span class="material-symbols-outlined text-9xl text-blue-600">school</span>
                    </div>
                    <div class="flex flex-col gap-1 relative z-10">
                        <div class="flex items-center gap-2 mb-2">
                             <div class="p-2 bg-blue-50/50 rounded-lg text-blue-600">
                                <span class="material-symbols-outlined">school</span>
                             </div>
                             <p class="text-xs font-bold uppercase tracking-wider text-slate-500">Tenaga Pendidik</p>
                        </div>
                        <h3 class="text-4xl font-extrabold text-slate-800 dark:text-white">{{ $stats['total_guru'] }}</h3>
                        <div class="flex items-center gap-1 mt-2 text-xs font-medium text-slate-500">
                             ...
                             {{ $stats['total_kelas'] }} Wali Kelas Aktif
                        </div>
                    </div>
                </div>

                <!-- Tahun Ajaran -->
                <div class="col-span-1 md:col-span-2 relative overflow-hidden rounded-2xl p-6 text-white shadow-xl shadow-primary/20 group">
                    <!-- Dynamic Gradient Background -->
                    <div class="absolute inset-0 bg-gradient-to-br from-primary via-primary-dark to-slate-900 z-0"></div>
                    <div class="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 z-0 mix-blend-overlay"></div>

                    <div class="absolute -right-10 -bottom-10 opacity-10 group-hover:opacity-20 transition-all duration-700 transform group-hover:scale-110">
                         <span class="material-symbols-outlined text-[180px]">calendar_month</span>
                    </div>

                    <div class="relative z-10 flex flex-col justify-between h-full">
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="text-primary-100 text-xs font-bold uppercase tracking-widest mb-1">Tahun Ajaran Aktif</p>
                                <h3 class="text-3xl font-bold tracking-tight">{{ $activeYear ? $activeYear->nama : 'Belum Ada' }}</h3>
                            </div>
                        ...
                        <div class="mt-4 flex items-center gap-3">
                            <span class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/10 backdrop-blur-sm border border-white/10 text-xs font-bold">
                                <span class="size-2 bg-emerald-400 rounded-full animate-pulse shadow-[0_0_10px_rgba(52,211,153,0.8)]"></span>
                                Semester Berjalan
                            </span>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="flex flex-col gap-5">
                     ...
                     <div class="card-boss p-8 flex flex-col gap-8 h-full justify-center">
                        @php
                        ...
                        <div class="relative px-2">
                            <div class="absolute top-1/2 left-0 w-full h-1.5 bg-slate-100 dark:bg-slate-700 -z-10 -translate-y-1/2 rounded-full"></div>

                            <!-- Active Line -->
                            <div class="absolute top-1/2 left-0 h-1.5 bg-primary -z-10 -translate-y-1/2 rounded-full transition-all duration-1000" style="width: {{ ($activeCawu - 1) * 50 }}%"></div>

                            <div class="flex justify-between w-full">
                                @foreach($cawuLabels as $index => $label)
                                    @php $step = $index + 1; @endphp
                                    <div class="flex flex-col items-center gap-3 relative z-10 group cursor-default">
                                        @if($step < $activeCawu)
                                            <div class="size-12 rounded-full bg-primary text-white flex items-center justify-center shadow-lg shadow-primary/20 transition-transform group-hover:scale-110 ring-4 ring-white dark:ring-surface-dark">
                                                <span class="material-symbols-outlined text-xl">check</span>
                                            </div>
                                        @elseif($step == $activeCawu)
                                            <div class="size-12 rounded-full bg-white border-2 border-primary text-primary flex items-center justify-center shadow-lg ring-4 ring-primary/10 animate-pulse-slow relative">
                                                <span class="absolute inset-0 rounded-full border border-primary opacity-50 animate-ping"></span>
                                                <span class="text-base font-bold">{{ $step }}</span>
                                            </div>
                                        @else
                                            <div class="size-12 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center border border-slate-200 ring-4 ring-white dark:ring-surface-dark">
                                                <span class="text-base font-bold">{{ $step }}</span>
                                            </div>
                                        @endif
                                        <span class="text-[10px] font-bold uppercase tracking-wider {{ $step == $activeCawu ? 'text-primary' : 'text-slate-400' }}">{{ $label }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mt-auto">
                            <div class="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
                                <span class="text-[10px] text-slate-500 font-bold uppercase tracking-wide mb-1">Kelas Dinilai</span>
                                <p class="text-2xl font-black text-slate-900 dark:text-white">
                                    {{ $miStats['finalized_classes'] }} <span class="text-slate-400 text-sm font-normal">/ {{ $miStats['total_classes'] }}</span>
                                </p>
                            </div>
                            <div class="p-4 rounded-xl border flex flex-col items-center text-center {{ is_null($miStats['days_left']) ? 'bg-slate-50 border-slate-100' : ($miStats['days_left'] < 7 ? 'bg-red-50 border-red-100 text-red-700' : 'bg-primary/5 border-primary/10 text-primary') }}">
                                <span class="text-[10px] font-bold uppercase tracking-wide mb-1 {{ is_null($miStats['days_left']) ? 'text-slate-500' : ($miStats['days_left'] < 7 ? 'text-red-600' : 'text-primary') }}">Deadline</span>
                                @if(is_null($miStats['days_left']))
                                     <p class="text-sm font-bold text-slate-400 mt-1">Belum Diset</p>
                                 @else
                                     <p class="text-2xl font-black">{{ $miStats['days_left'] }} <span class="text-[10px] font-bold uppercase opacity-80">Hari</span></p>
                                 @endif
                            </div>
                        </div>
                     </div>
                ...
                <div class="flex flex-col gap-5">
                    <h3 class="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-3">
                     ...
                     <div class="card-boss p-6 border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col justify-between h-full gap-6">
                        <div class="flex flex-col gap-4">
                             <!-- Semesters -->
                            <div class="relative overflow-hidden rounded-xl border {{ $mtsStats['active_semester'] == 1 ? 'border-primary shadow-lg shadow-primary/5 bg-primary/5' : 'border-slate-100 bg-slate-50/50' }} p-4 flex items-center justify-between transition-all group hover:scale-[1.02] duration-300">
                                <div class="flex items-center gap-4">
                                    <div class="size-10 rounded-xl {{ $mtsStats['active_semester'] == 1 ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-white border border-slate-200 text-slate-400' }} flex items-center justify-center font-bold text-base transition-colors">1</div>
                                    <div>
                                        <p class="font-bold text-slate-900 dark:text-white text-sm">Semester Ganjil</p>
                                        <p class="text-[11px] text-slate-500 uppercase tracking-wide font-medium">{{ $mtsStats['active_semester'] == 1 ? 'Sedang Berjalan' : ($mtsStats['active_semester'] > 1 ? 'Selesai' : 'Belum Mulai') }}</p>
                                    </div>
                                </div>
                                @if($mtsStats['active_semester'] == 1)
                                <div class="px-2 py-0.5 bg-white text-primary rounded-md text-[10px] font-bold border border-primary/20 shadow-sm">AKTIF</div>
                                @endif
                            </div>

                            <div class="relative overflow-hidden rounded-xl border {{ $mtsStats['active_semester'] == 2 ? 'border-primary shadow-lg shadow-primary/5 bg-primary/5' : 'border-slate-100 bg-slate-50/50' }} p-4 flex items-center justify-between transition-all group hover:scale-[1.02] duration-300">
                                <div class="flex items-center gap-4">
                                    <div class="size-10 rounded-xl {{ $mtsStats['active_semester'] == 2 ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-white border border-slate-200 text-slate-400' }} flex items-center justify-center font-bold text-base transition-colors">2</div>
                                    <div>
                                        <p class="font-bold text-slate-900 dark:text-white text-sm">Semester Genap</p>
                                        <p class="text-[11px] text-slate-500 uppercase tracking-wide font-medium">{{ $mtsStats['active_semester'] == 2 ? 'Sedang Berjalan' : 'Belum Mulai' }}</p>
                                    </div>
                                </div>
                         ...
                         <div class="mt-auto p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl text-center border border-slate-100 dark:border-slate-800 relative overflow-hidden">
                            <div class="absolute inset-0 bg-grid-slate-200/50 [mask-image:linear-gradient(0deg,white,rgba(255,255,255,0.6))]"></div>
                            <div class="relative z-10">
                                <p class="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">Estimasi Pembagian Rapor</p>
                                <p class="text-lg font-bold text-slate-900 dark:text-white">
                                     @if($mtsStats['deadline'])
                                        {{ \Carbon\Carbon::parse($mtsStats['deadline'])->translatedFormat('d F Y') }}
                                     @else
                                        <span class="text-slate-400 italic font-normal">Belum dijadwalkan</span>
                                     @endif
                                </p>
                            </div>
                         </div>
             ...
             <div class="flex flex-col gap-4 pb-8" x-data="{
                ...
                <div class="flex items-center justify-between">
                    ...
                    <button @click="showBulkModal = true" class="btn-boss btn-secondary text-xs" title="Ingatkan Semua">
                        <span class="material-symbols-outlined text-[18px]">notifications_active</span> Ingatkan Semua
                    </button>
                </div>

                 <div class="card-boss overflow-hidden flex flex-col max-h-[500px]">
                    <div class="overflow-y-auto custom-scrollbar">
                        <table class="w-full text-left text-sm whitespace-nowrap">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                ...
                                <tr class="table-row">
                                    <td class="table-cell font-bold text-slate-900 dark:text-white">
                                        {{ $kelas['nama_kelas'] }}
             ...
             <div class="flex flex-col gap-4 pb-8" x-data="{ showReminderModal: false, selectedWaliId: '', selectedKelasName: '', reminderMessage: '', messageType: 'warning' }">
                ...
                <!-- Reminder Modal -->
                <div x-show="showReminderModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm px-4"
                    ...
                    <div class="bg-white dark:bg-surface-dark rounded-2xl shadow-xl w-full max-w-md p-6 m-4 border border-slate-100 dark:border-slate-800"
                        ...
                        <form action="{{ route('dashboard.remind-wali') }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="message" x-model="reminderMessage" rows="3" class="input-boss" placeholder="Tulis pesan Anda di sini..."></textarea>
                            </div>
