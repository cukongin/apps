...
@section('content')
<div class="flex flex-col min-h-[calc(100vh-80px)]" x-data="promotionPage()">
    <!-- Header -->
    ...
    <!-- CRITERIA INFO CARD -->
    <div class="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6 rounded-r-lg shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h3 class="font-bold text-blue-700 flex items-center gap-2">
                <span class="material-symbols-outlined">info</span>
                Syarat Kenaikan / Kelulusan ({{ $selectedClass->jenjang->kode ?? 'Jalur Umum' }})
            </h3>
            <p class="text-sm text-blue-600 mt-1">
                Siswa dinyatakan <b>LULUS/NAIK</b> jika memenuhi kriteria berikut:
            </p>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div class="bg-white/60 p-2 rounded border border-blue-100">
                <span class="block text-xs text-blue-500 font-bold uppercase">Min. Kehadiran</span>
                <span class="font-bold text-slate-700">{{ $criteria->promotion_min_attendance }}%</span>
            </div>
            <div class="bg-white/60 p-2 rounded border border-blue-100">
                <span class="block text-xs text-blue-500 font-bold uppercase">Max. Mapel < KKM</span>
                <span class="font-bold text-slate-700">{{ $criteria->promotion_max_kkm_failure }} Mapel</span>
            </div>
            <div class="bg-white/60 p-2 rounded border border-blue-100">
                <span class="block text-xs text-blue-500 font-bold uppercase">Min. Sikap</span>
                <span class="font-bold text-slate-700">Minimal {{ $criteria->promotion_min_attitude }}</span>
            </div>
            <div class="bg-white/60 p-2 rounded border border-blue-100">
                <span class="block text-xs text-blue-500 font-bold uppercase">KKM Default</span>
                <span class="font-bold text-slate-700">{{ $criteria->kkm_default }}</span>
            </div>
        </div>
    </div>
                    ...
                    <tr x-show="matchesSearch('{{ strtolower($st->nama_siswa) }}')" class="bg-white dark:bg-[#1a2332] hover:bg-slate-50 dark:hover:bg-[#1e2837] transition-colors group">
                        ...
                        <td class="px-6 py-4 font-medium text-slate-900 dark:text-white whitespace-nowrap">
                         ...
                         <td class="px-6 py-4 text-center">
                            @if($st->kkm_failure_count > 0)
                                <span class="text-red-600 font-bold bg-red-50 px-2 py-0.5 rounded">{{ $st->kkm_failure_count }}</span>
                            @else
                        ...
                        <td class="px-6 py-4 text-right">
                           ...
                           <!-- NOTES / ALASAN -->
                           @if($st->notes)
                                <div class="mt-1 text-[10px] text-red-500 leading-tight font-medium bg-red-50 p-1 rounded border border-red-100">
                                    {{ Str::limit($st->notes, 50) }}
                                </div>
                           @endif
                        </td>
                    </tr>
...
<script>
        ...
        return {
            ...
            async bulkUpdate(status) {
                 const result = await Swal.fire({
                    title: 'Update Massal?',
                    text: `Yakin set status ${status.toUpperCase().replace('_', ' ')} untuk ${this.selectedIds.length} santri terpilih?`,
                    icon: 'question',
                ...
                if (!result.isConfirmed) return;
                 ...
                 try {
                     ...
                     if (res.ok) {
                         ...
                         Swal.fire('Gagal!', 'Gagal melakukan update massal', 'error');
                     }
                 } catch(e) {
                     Swal.fire('Error!', 'Terjadi kesalahan jaringan.', 'error');
                 }
    ...
    function confirmProcessAll() {
        Swal.fire({
            title: 'Hitung Ulang Semua?',
            text: 'Proses ini akan menghitung ulang rekomendasi kenaikan untuk SEMUA KELAS. Pastikan semua nilai sudah masuk.',
            icon: 'warning',
            showCancelButton: true,
    ...
    function confirmFinalize() {
        Swal.fire({
            ...
            html: 'Apakah Anda yakin ingin <b>MENGUNCI PERMANEN</b> status kenaikan kelas saat ini?<br><br>Setelah dikunci, tombol "Hitung Semua" <b>TIDAK AKAN</b> mengubah data lagi.<br>Ini adalah langkah akhir sebelum cetak Rapor/Leger.',
            icon: 'warning',
            showCancelButton: true,
