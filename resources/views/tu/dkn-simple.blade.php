...
<div class="flex-1 flex flex-col h-full overflow-hidden relative bg-slate-50 dark:bg-slate-900">
    ...
    <!-- Top Bar -->
    <div class="px-6 py-5 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-700/50 flex flex-col md:flex-row justify-between md:items-center gap-4 z-20 sticky top-0">
        ...
        <div class="flex items-center gap-3">
             <a href="{{ route('tu.dkn.archive', $kelas->id) }}" class="btn-boss bg-white border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-2 shadow-sm">
    ...
    <div class="px-4 md:px-6 py-4 md:py-6 flex overflow-x-auto snap-x md:grid md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 pb-6 md:pb-6 no-scrollbar">
        <!-- Average Card -->
        <div class="min-w-[85%] md:min-w-0 flex-shrink-0 snap-center card-boss !p-5 relative overflow-hidden group hover:shadow-md transition-all">
            ...
            <div class="flex items-baseline gap-2">
                ...
                <span class="text-[10px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 dark:text-emerald-400 px-2 py-0.5 rounded-full">NA</span>
            </div>
    ...
    <div class="flex-1 overflow-hidden px-4 pb-4 md:px-6 md:pb-6" x-data>
        <div class="card-boss !p-0 relative h-full flex flex-col overflow-hidden shadow-xl shadow-slate-200/50 dark:shadow-slate-900/50">
            <div class="overflow-auto flex-1 custom-scrollbar">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 sticky top-0 z-40 shadow-sm border-b border-slate-200 dark:border-slate-700">
                        <tr>
                            <th rowspan="2" class="p-3 text-center text-xs font-bold uppercase border-r border-slate-200 dark:border-slate-700 w-12 md:sticky md:left-0 z-50 bg-slate-50 dark:bg-slate-800">No</th>
                            <th rowspan="2" class="p-3 text-left text-xs font-bold uppercase border-r border-slate-200 dark:border-slate-700 w-64 min-w-[200px] md:min-w-[250px] md:sticky md:left-12 z-50 bg-slate-50 dark:bg-slate-800 shadow-[4px_0_8px_rgba(0,0,0,0.05)]">
                                Peserta Didik
                            </th>
                            @foreach($mapels as $mapel)
                                <th colspan="3" class="px-2 py-2 text-center border-r border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 last:border-r-0">
                                    <div class="text-[11px] font-bold text-slate-700 dark:text-slate-300 truncate max-w-[120px]" title="{{ $mapel->nama_mapel }}">{{ $mapel->nama_mapel }}</div>
                                </th>
                            @endforeach
                            <th rowspan="2" class="p-3 text-center text-xs font-bold uppercase border-l border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 md:sticky md:right-[100px] z-40 min-w-[80px] shadow-[-4px_0_8px_rgba(0,0,0,0.05)]">RataÂ²</th>
                            <th rowspan="2" class="p-3 text-center text-xs font-bold uppercase border-l border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 md:sticky md:right-0 z-40 min-w-[100px]">Status</th>
                        </tr>
                        <tr>
                            @foreach($mapels as $mapel)
                                <th class="py-1 px-1 text-center text-[10px] font-semibold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700/50 border-r border-slate-200 dark:border-slate-700 border-t border-slate-200 dark:border-slate-700 w-14">RR</th>
                                <th class="py-1 px-1 text-center text-[10px] font-semibold text-slate-500 dark:text-slate-400 bg-amber-50/50 dark:bg-amber-900/20 border-r border-slate-200 dark:border-slate-700 border-t border-slate-200 dark:border-slate-700 w-14">UM</th>
                                <th class="py-1 px-1 text-center text-[10px] font-semibold text-slate-500 dark:text-slate-400 bg-emerald-50/50 dark:bg-emerald-900/20 border-r border-slate-200 dark:border-slate-700 border-t border-slate-200 dark:border-slate-700 w-14">NA</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                        @forelse($dknData as $index => $row)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors group">
                                <td class="p-3 text-center text-xs font-bold text-slate-500 dark:text-slate-400 border-r border-slate-100 dark:border-slate-700 md:sticky md:left-0 bg-white dark:bg-slate-800 z-30 group-hover:bg-slate-50 dark:group-hover:bg-slate-700/30">
                                    {{ $index + 1 }}
                                </td>
                                <td class="p-3 border-r border-slate-100 dark:border-slate-700 md:sticky md:left-12 bg-white dark:bg-slate-800 z-30 shadow-[4px_0_8px_rgba(0,0,0,0.03)] group-hover:bg-slate-50 dark:group-hover:bg-slate-700/30">
                                    <div class="flex flex-col">
                                ...
                                @foreach($mapels as $mapel)
                                    ...
                                    <td class="border-r border-slate-100 dark:border-slate-700 p-1 text-center bg-emerald-50/10 dark:bg-emerald-900/10">
                                        @if($na !== null)
                                            <span class="text-[11px] font-bold {{ $na < 75 ? 'text-red-500' : 'text-emerald-600 dark:text-emerald-400' }}">
                                ...
                                <td class="p-3 text-center border-l border-slate-100 dark:border-slate-700 md:sticky md:right-0 z-30 bg-white dark:bg-surface-dark group-hover:bg-slate-50 dark:group-hover:bg-slate-700/30">
                                     @if($avg > 0)
                                        @if($isPass)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800">
                                                LULUS
                                            </span>
                                        @else
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-100 text-rose-800 border border-rose-200 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800">
                                                BELUM
                                            </span>
                                        @endif
                                     @else
